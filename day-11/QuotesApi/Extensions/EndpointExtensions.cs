...
auth.MapPost("/refresh", async (RefreshRequest request, QuotesDbContext db, IJwtTokenService tokenService, IOptions<JwtOptions> jwtOptions, ILogger<Program> logger, CancellationToken ct) =>
{
    ...
    return Results.Ok(new LoginResponse
    {
        ...
        // Profiling exercise: deliberately unoptimised. One query for the distinct
        // author list, then a SEPARATE query per author (N+1) that sorts by the
        // unindexed Text column (forces a table scan) instead of using Include,
        // a join, or GroupBy. Do not fix this - the slowness is the point.
        authors.MapGet("/slow", async (QuotesDbContext db, CancellationToken ct) =>
        {
            var authorNames = await db.Quotes
                .Select(q => q.Author)
                .Distinct()
                .ToListAsync(ct);
            ...
            foreach (var author in authorNames)
            {
                ...
                summaries.Add(new
                {
                    ...
                    firstQuote = authorQuotes.FirstOrDefault()?.Text
                });
            }
