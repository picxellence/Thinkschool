...
auth.MapPost("/refresh", async (RefreshRequest request, QuotesDbContext db, IJwtTokenService tokenService, IOptions<JwtOptions> jwtOptions, ILogger<Program> logger, CancellationToken ct) =>
{
    ...
    return Results.Ok(new LoginResponse
    {
        ...
        // Command path: validates, stamps the owner, and persists through the
        // domain entity. Returns only the write outcome - never a read-shaped DTO.
        collections.MapPost("", async (CreateCollectionRequest request, ClaimsPrincipal user, CreateCollectionCommandHandler handler, CancellationToken ct) =>
        {
            var ownerUserId = user.FindFirst("oid")?.Value ?? user.FindFirst("sub")?.Value;

            var result = await handler.HandleAsync(new CreateCollectionCommand(request.Name, ownerUserId), ct);
            if (!result.Succeeded)
                return Results.ValidationProblem(result.Errors);

            return Results.Created($"/collections/{result.Id}", new
            {
                id = result.Id,
                name = result.Name,
        ...
        // Query path: a single read-only projection shaped for display, no
        // domain entity materialized, shares no model with the command above.
        collections.MapGet("/{id:int}/summary", async (int id, CollectionSummaryQueryHandler handler, CancellationToken ct) =>
        {
